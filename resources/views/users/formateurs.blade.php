 ...
 @if ($message = Session::get('success'))
    ...
    @endif
        ...
        @foreach($users as $user)
                  <td> {{$user->name}}</td>
                  <td style="background-color: #DBDBDB; text-align: center; font-size: 15px;">{{$user->email}}
                  </td>
                   <td>{{$user->created_at}}</td>
                   <td>@if ($user->payed==0 )
                        <h4 style="color: red;"><b>Non Payé(e)</b> </h4>
                        @endif
                        @if ($user->payed==1)
                        <h4 style="color: green;"><b>Payé(e)</b></h4>
                        @endif

                  </td>
        <td> 
            <a title="supprimer" type="button" class="btn btn-danger " data-toggle="modal" data-target="#edit{{ $user->id }}" id="edit{{ $user->id }}"><i class="fa fa-trash-o" ></i></a>
        </td>
    </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Voulez vous supprimer cet utilisateur</h4>
      </div>
