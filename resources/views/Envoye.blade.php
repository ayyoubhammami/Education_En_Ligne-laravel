...
<!-- Main content -->
    <section class="content">
      <div class="row" style="margin-top: 50px;">
        <h3>Liste des Messages envoyées</h3>
        <div class="col-md-3">
          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example2" class="table table-hover table-responsive table-striped">
               ...
               <tbody>
                    @foreach(App\Message::where('sender',Auth::user()->id)->orderBy('created_at','desc')->get() as $message)
                    @if($message->Remouve==1)
                  <tr> 
                    <td>
                       <a href="{{route('remouve',['id'=>$message->id])}}" type="button" class="btn btn-default btn-sm"><i class="fa fa-trash-o"></i></a>
                    </td>
                    
                    <td class="mailbox-name">
                      @if(App\User::find($message->reciever)->role_id ==3)
                        <i class="fa fa-star text-yellow"></i>&nbsp;
                      @endif
                      @if(App\User::find($message->reciever)->role_id ==2)
                        <i class="fa fa-star text-default"></i>&nbsp;
                      @endif
                      @if(App\User::find($message->reciever)->role_id ==1)
                        <i class="fa fa-star text-yellow"></i>&nbsp;
                      @endif
                      <a href="{{route('EnvoyeMessage',['id'=>$message->id])}}">{{App\User::find($message->reciever)->name}}</a><br><small>{{App\User::find($message->reciever)->email}}</small></td>
            ...
            <h3 style="color: red;">Vous avez caché ce message <i class="fa fa-eye"></i></h3></a>
                  @endif
                  @endforeach
                  </tbody>
              </table>
